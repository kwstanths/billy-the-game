...
int World::Init(Input * input, Camera * camera, ge::GameEngine * engine) {
    ...
    wall->Init(-1.0f, 6.5f, 0.01f, engine);
    wall = NewObj<Wall>(true);
    wall->Init(0.0f, 6.5f, 0.01f, engine);
    wall = NewObj<Wall>(true);
    wall->Init(1.0f, 6.5f, 0.01f, engine);
    wall = NewObj<Wall>(true);
    wall->Init(2.0f, 6.5f, 0.01f, engine);
    wall = NewObj<Wall>(true);
    wall->Init(3.0f, 6.5f, 0.01f, engine);
    wall = NewObj<Wall>(true);
    wall->Init(4.0f, 6.5f, 0.01f, engine);

    /* Create some collidable treasures with some random ids */
    Treasure * tres = NewObj<Treasure>();
    tres->Init(0.0f, 1.5f, 0.01f, engine, 1);
    tres = NewObj<Treasure>();
    tres->Init(-2.4f, 1.5f, 0.01f, engine, 2);
    tres = NewObj<Treasure>();
    tres->Init(5.0f, -5.0f, 0.01f, engine, 3);
    tres = NewObj<Treasure>();
    tres->Init(5.0f, 4.94f, 0.01f, engine, 4);
    tres = NewObj<Treasure>();
    tres->Init(6.1f, 6.0f, 0.01f, engine, 5);
    tres = NewObj<Treasure>();
