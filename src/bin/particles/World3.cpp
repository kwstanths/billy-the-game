...
World3::World3() : WorldSector() {
    is_inited_ = false;
}

int World3::Init(Input * input, Camera * camera, ge::GameEngine * engine) {
    ...
    for (size_t i = 0; i < rows; i++) {
        for (size_t j = 0; j < cols; j++) {
            ...
            p->Init(Particle::SpawningMethod::RAND_VELOCITY, glm::vec3(-1.0f + hd * i, 6 - vd * j, 5 -0.2*j), &rng_, &planes_, &triangles_, &spheres_, this, engine);
