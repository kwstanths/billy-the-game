...
World1::World1() : WorldSector() {
    is_inited_ = false;
}

int World1::Init(Input * input, Camera * camera, ge::GameEngine * engine) {
    ...
    for (size_t i = 0; i < rows; i++) {
        for (size_t j = 0; j < cols; j++) {
            ...
            p->Init(Particle::SpawningMethod::NONE, glm::vec3(-2.5f + hd * i, 6, -2.5f + vd * j), &rng_, &planes_, &triangles_, &spheres_, this, engine);
            p->SetForce(glm::vec3(0, -3, 0));
            p->SetVelocity(glm::vec3(0, 0, 0));
            p->SetSpringElasticity(0.3);
    ...
    sphere->spring_elasticity_k_ = 0.3;
