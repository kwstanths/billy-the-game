...
namespace graphics {
    ...
    void Renderer::AddDropplet(glm::vec2 drop)
    {
        renderer_->droplet_buffer_.Push(drop);
    }

    float Renderer::GetAverageWaterDisplacement()
    {
        return renderer_->average_displacement_;
    }

    int Renderer::DrawWater(GraphicsObject * rendering_object, bool update_water) {
        rendering_object->SetModelMatrix();
        ...
        renderer_->g_buffer_->Bind();
        Mesh * mesh = rendering_object->model_->meshes_[0];
        renderer_->DrawGBufferWater(mesh->opengl_object_, mesh->opengl_textures_, rendering_object->model_matrix_, mesh->mat_);
        renderer_->g_buffer_->UnBind();
    ...
    void Renderer::FlushDrawCalls() {
        ...
        //renderer_->DrawTexture(renderer_->frame_buffer_water_one_->output_texture_);
        //return;
