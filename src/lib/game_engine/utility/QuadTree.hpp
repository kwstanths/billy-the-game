...
namespace utility {
    ...
    private:
        ...
        public:
            ...
            virtual QuadTreeNode * Remove(Vector2D point) = 0;
        ...
        public:
            QuadTreeLeafNode(Vector2D origin, Real_t length) {
                this->origin_ = origin;
                this->length_ = length;
            ...
            QuadTreeNode * Insert(Vector2D point, Data data, size_t depth) {
            ...
            QuadTreeLeafNode * Remove(Vector2D point) {
        ...
        private:
            /* Holds a data point */
            struct Bucket {
                Bucket(Vector2D point, Data data) : point_(point), data_(data) {};
                Vector2D point_;
                Data data_;
            };
        ...
        public:
            QuadTreeInnerNode(Vector2D origin, Real_t length) {
    ...
    public:
        ...
        */
        bool Insert(Vector2D point, Data data) {
        ...
        */
        void QueryRange(AABox<2> search_box, std::vector<Data>& results) {
            ...
            if (search_box.Overlaps(AABox<2>(origin_, origin_ + Vector2D(length_))))
        ...
        */
        void RayCast(Ray2D r, std::vector<Data>& results) {
            ...
            */
            if (r.Direction()[0] < 0) {
                r.Origin()[0] = (origin_[0] + length_ / Real_t(2)) * Real_t(2) - r.Origin()[0];
                r.Direction()[0] = -r.Direction()[0];
                a |= 2;
            }
            if (r.Direction()[1] < 0) {
                r.Origin()[1] = (origin_[1] + length_ / Real_t(2)) - r.Origin()[1];
